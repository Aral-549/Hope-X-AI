"""
generate_review_html_502.py
===========================
Generates a browser-based review contact sheet for the 502 hardest boundary cases.
"""
...
queue_path = PROJECT_ROOT / "scratch/hard_examples_queue.json"
html_out = PROJECT_ROOT / "scratch/review_sheet_final502.html"
...
<head>
<meta charset="UTF-8">
<title>HackBlox 2026 - Final 502 Hard Boundary Review</title>
<style>
  ...
  .card img {{ width: 100%; height: 180px; object-fit: cover; background: #000; cursor: pointer; }}
...
<div class="header">
  <div>
    <h1 style="margin:0; font-size: 20px;">HackBlox 2026 · Final 502 Hard Boundary Review</h1>
    <div style="font-size: 13px; color: #94a3b8;">Uniform category review. Models are least confident on these samples. Select the true class.</div>
  </div>
  <div style="display: flex; gap: 10px; align-items: center;">
    <span id="counter" style="font-weight: 600; color: #38bdf8;">0 / {len(queue)} reviewed</span>
    <button class="btn" onclick="exportCSV()">Export final_human_labels.csv</button>
  </div>
</div>
...
for idx, item in enumerate(queue):
    ...
    t1 = item["predicted_class_idx"]
    t2 = item["top2_class_idx"]
  ...
  <div class="card" id="card-{item['id']}">
    ...
    <div class="card-body">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <span style="font-weight: bold;">#{idx + 1} (ID: {item['id']})</span>
        <span class="badge">Entropy: {item['entropy']:.2f}</span>
      </div>
      <div class="preds">
        Top Candidates: <b>[{t1}] {item['predicted_class']}</b> ({item['confidence']:.2f}) vs <b>[{t2}] {item['top2_class']}</b>
      </div>
      <div class="options">
"""
...
function exportCSV() {{
  let csv = "id,image_path,label,label_name,weight,source\\n";
  ...
  queue.forEach(item => {{
    ...
    if (checked) {{
      ...
      csv += `${{item.id}},"${{item.image_path}}",${{lbl}},${{classNames[lbl]}},1.0,expert_human_final\\n`;
    }}
  }});
  ...
  a.setAttribute('download', 'final_human_labels.csv');
  a.click();
}}
...
with open(html_out, "w") as f:
    f.write(html)
print(f"[OK] Generated review HTML sheet: {html_out}")
