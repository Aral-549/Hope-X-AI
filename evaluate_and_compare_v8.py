"""
evaluate_and_compare_v8.py
==========================
Compares 3-seed ensembles for:
- train_0005 (baseline clean: 2,498 samples)
- train_0006 (flawed auto-script: 2,696 samples)
- train_0008 (genuine visual inspection: 2,971 samples)

Prints per-class recall for all 6 classes and overall validation accuracy.
"""
...
def main():
    ...
    ckpts_v8 = [
        "scratch/clean_model_seed42_v8.pth",
        "scratch/clean_model_seed43_v8.pth",
        "scratch/clean_model_seed44_v8.pth",
    ]

    print("=" * 70)
    print("  Evaluating 3-Seed Ensembles: train_0005 vs train_0006 vs train_0008")
    print("=" * 70)

    p_v5, targets = evaluate_models(ckpts_v5, val_loader)
    p_v6, _ = evaluate_models(ckpts_v6, val_loader)
    p_v8, _ = evaluate_models(ckpts_v8, val_loader)

    if p_v8 is None:
        print("[ERROR] train_0008 models not ready yet.")
        sys.exit(1)

    acc_v5 = 100.0 * (p_v5.argmax(axis=1) == targets).mean()
    acc_v6 = 100.0 * (p_v6.argmax(axis=1) == targets).mean()
    acc_v8 = 100.0 * (p_v8.argmax(axis=1) == targets).mean()
    ...
    rec_v8 = recall_score(targets, p_v8.argmax(axis=1), average=None) * 100

    delta_v8_v5 = rec_v8 - rec_v5
    delta_v8_v6 = rec_v8 - rec_v6

    df_comp = pd.DataFrame({
        "Class": CLASSES,
        "train_0005": rec_v5.round(1),
        "train_0006": rec_v6.round(1),
        "train_0008": rec_v8.round(1),
        "Delta (v8 - v5)": delta_v8_v5.round(1),
        "Delta (v8 - v6)": delta_v8_v6.round(1),
    })
    ...
    print("-" * 70)
    print(f"Overall Accuracy:  train_0005: {acc_v5:.2f}%  |  train_0006: {acc_v6:.2f}%  |  train_0008: {acc_v8:.2f}%")
    print("=" * 70)

    # Save summary
    df_comp.to_csv(PROJECT_ROOT / "scratch/per_class_recall_comparison_v8.csv", index=False)

    print("\nDetailed Classification Report for train_0008 3-seed Ensemble:")
    print(classification_report(targets, p_v8.argmax(axis=1), target_names=CLASSES, digits=4))
